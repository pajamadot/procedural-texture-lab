...
OpenAI is used for procedural code-node generation. Fal is used for non-procedural bitmap texture generation through `POST /api/generate-image-texture`.
...
Image texture nodes are non-procedural bitmap textures generated by Fal. They are sampled by UV in the render engine, so they can feed into operation nodes and exported texture output just like procedural code nodes.
